...
void add_round_key(uint8_t *state, uint8_t *w, uint8_t r) {
    ...
    printf("\nRound %d Key:\n",r);
    for (c = 0; c < 4; c++) {
        printf("%x %x %x %x ", w[4 * Nb*r + 4 * c + 0], w[4 * Nb*r + 4 * c + 1], w[4 * Nb*r + 4 * c + 2], w[4 * Nb*r + 4 * c + 3]);//Column
    }
...
void cipher(uint8_t *in, uint8_t *out, uint8_t *w) {
    ...
    */
    ...
    printf("\nRound 0:\n");
    for (i = 0; i < 4; i++) {
        printf("%x %x %x %x ", state[4 * 0 + i], state[4 * 1 + i], state[4 * 2 + i], state[4 * 3 + i]);//Column
    }
...
//         0x44, 0x55, 0x66, 0x77,
//         0x88, 0x99, 0xaa, 0xbb,
//         0xcc, 0xdd, 0xee, 0xff }; // 128
//         
	uint8_t in[] = {
	0xeb,0x11,0x67,0xb3,0x67,0xa9,0xc3,0x78,0x7c,0x65,0xc1,0xe5,0x82,0xe2,0xe6,0x62};
...
//         0x08, 0x09, 0x0a, 0x0b,
//         0x0c, 0x0d, 0x0e, 0x0f };
uint8_t key[] = { 0xe3,0xe7,0x06,0x82,0xc2,0x09,0x4c,0xac,0x62,0x9f,0x6f,0xbe,0xd8,0x2c,0x07,0xcd};
    /* 192 bit key */
    /* uint8_t key[] = {
        0x00, 0x01, 0x02, 0x03,
        0x04, 0x05, 0x06, 0x07,
